    ...
    if (mapData.length > 0) {
      ...
      mapData.forEach(log => {
        if (log.location && !isNaN(log.location.lat) && !isNaN(log.location.lng)) {
          // If it isn't literally 0,0 default or we want to show 0,0 anyway
  ...
  return (
    <div className="absolute inset-0 w-full h-full flex">
      ...
      <div className="flex-1 h-full relative">
        ...
        {mapData.length === 0 && (
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white/90 backdrop-blur-sm p-4 rounded-2xl shadow-xl border border-gray-100 text-center pointer-events-none z-10 w-3/4 max-w-sm">
      ...
      {/* Floating Side Panel */}
      <div className="w-80 h-full bg-white/90 backdrop-blur-md shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.1)] border-l border-gray-100 flex flex-col z-[5] overflow-hidden hidden md:flex absolute right-0 top-0 bottom-0">
